...
# Change this to logging.DEBUG, logging.WARNING, etc.
...
rate_handler.addFilter(lambda record: record.levelno ==
                       logging.WARNING and "rate limit hit" in record.getMessage())
        ...
        try:
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    logger.info(
                        f"Response for movie ID {movie_id} saved to: {file_path}")
                    items_scraped += 1
                    ...
                    logger.error(
                        f"Movie ID {movie_id} returned 404. Marking as not found.")
                    # Write a file in the 404/ directory to record the known 404
                    error_file_path = os.path.join(
                        error_dir, f"{movie_id}.txt")
                    ...
                    logger.error(
                        f"Movie ID {movie_id} returned status code: {response.status}")
                    consecutive_404 = 0
                    break
...
async def main():
    ...
    logger.info(
        f"Found {len(existing_ids)} existing files and {len(known_404_ids)} known 404s. {missing_count} missing files in range {start_id} to {end_id}.")
            ...
            for movie_id in missing_ids_generator(start_id, end_id, existing_ids, known_404_ids):
                ...
                if len(tasks) >= max_requests_per_second:
                    ...
                    if consecutive_404 >= 5000:
                        logger.info(
                            f"Encountered {consecutive_404} consecutive 404 errors. Stopping further processing.")
                        break
